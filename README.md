...
| `ZMDG_S_SG_ACCT_NAMING` | UIBB structure: `COMP_CODE`, `ACCOUNT_GROUP`, `BANK`, `CURRENCY`, `PAYMENT_METHOD`. |
...
* `GENERATE_NUMBER` only builds an ID when **all five** structure fields are filled (`COMP_CODE`, `ACCOUNT_GROUP`, `BANK`, `CURRENCY`, `PAYMENT_METHOD`); otherwise it returns nothing.

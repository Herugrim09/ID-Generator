...
* Position 1-3 – group: the user picks the group **code** itself in the `ACCOUNT_GROUP` field OVS (`288` Main / `484` Interim In / `485` Interim Out / `253` IHB / `488` Interim IHB) – the OVS row's identifier is the code, its text is the plain description (no "kind" indirection, no code repeated in the text). `ACCOUNT_GROUP` is typed `ZE_MDG_ACCT_GROUP` (the 3-char group code), not `ACCT_GROUP_KIND`. A symbolic kind (`MAIN`/`INT_IN`/`INT_OUT`/`IHB`/`IHB_INT`) is still used internally to route the position-8 rule below, derived from the picked code via `ZCL_MDG_SG_ACCT_ID_GEN=>KIND_OF_GROUP`.
